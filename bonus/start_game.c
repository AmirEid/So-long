...
/*   Updated: 2024/02/18 22:18:24 by aeid             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	generate_images(t_map *mlx)
{
	...
	mlx->imags[2].img = mlx_xpm_file_to_image(mlx->mlxptr, ENEMY, &size, &size);
...
void	upload_images_to_window(t_map *map, int x, int y)
{
	if (map->map[x][y] == 'P')
		mlx_put_image_to_window(map->mlxptr, map->window, map->imags[3].img, y
			* 50, x * 50);
	if (map->map[x][y] == 'N')
		mlx_put_image_to_window(map->mlxptr, map->window, map->imags[2].img, y
			* 50, x * 50);
	if (map->map[x][y] == '1')
